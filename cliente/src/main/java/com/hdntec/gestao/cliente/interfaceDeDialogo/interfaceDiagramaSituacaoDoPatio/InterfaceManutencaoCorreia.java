 ...
 * @author guilherme
 */
public abstract class InterfaceManutencaoCorreia extends javax.swing.JPanel {
	...
	private List<ManutencaoCorreia> listaManutencoes;

	private List<ManutencaoCorreia> listaManutencoesRemovidas = new ArrayList<ManutencaoCorreia>();
	...
	private ManutencaoCorreia manutencaoSelecionada;

	private InterfaceCorreia maquina;
	
	/** Creates new form InterfaceManutencao */
	public InterfaceManutencaoCorreia(ControladorDSP controladorDSP, InterfaceCorreia maquina) {
				
		this.maquina = maquina;
		this.controladorDSP = controladorDSP;
		
		if (listaManutencoes == null) {
            listaManutencoes = new ArrayList<ManutencaoCorreia>();
            listaManutencoes.addAll(this.maquina.getCorreiaVisualizada().getMetaCorreia().getListaManutencao());
	...
	private void btnConfirmarActionPerformed(java.awt.event.ActionEvent evt) {
		...
		maquina.getCorreiaVisualizada().getMetaCorreia().getListaManutencao().clear();
		maquina.getCorreiaVisualizada().getMetaCorreia().addManutencao(listaManutencoes);
		
		MetaCorreiaDAO dao = new MetaCorreiaDAO();		
		try {            
            for (ManutencaoCorreia m : listaManutencoesRemovidas) {
                m.setCorreia(null);                
            }                
            dao.salvaMetaCorreia(maquina.getCorreiaVisualizada().getMetaCorreia());
	...
	private void btnConcluirEdicaoActionPerformed(java.awt.event.ActionEvent evt) {
		try {
			this.validaDados();
			ManutencaoCorreia manutencao = new ManutencaoCorreia();
